...
public class OpenGLRenderingView extends GLSurfaceView {
    ...
    public OpenGLRenderingView(Context context, AttributeSet attrs) {
        ...
        setRenderMode(GLSurfaceView.RENDERMODE_WHEN_DIRTY); // chỉ render khi requestRender()
    }
    ...
    public static class RectanglesRenderer implements GLSurfaceView.Renderer {
        ...
        private static final float[] COLOR = {0.2f, 0.7f, 1.0f, 0.8f}; // RGBA
        ...
        public void onSurfaceChanged(GL10 gl, int width, int height) {
            ...
            surfaceW = width;
            surfaceH = height;
            buildVbo(); // tạo/recreate VBO theo kích thước thực của màn hình
        }
        ...
        private void buildVbo() {
            ...
            float maxX = 2f - sizeNdcX; // NDC span 2 đơn vị (-1..1)
            float maxY = 2f - sizeNdcY;

            float[] vertices = new float[RECT_COUNT * 6 * 3];
            int idx = 0;
            for (int i = 0; i < RECT_COUNT; i++) {
                // Random trong [-1, 1 - size], để nằm trong màn
            ...
            Log.d("OpenGLRenderer", "VBO built with " + RECT_COUNT + " rects");
        }

        @Override
        public void onDrawFrame(GL10 gl) {
            if (hasMeasured) return;
            if (vboId == 0) return; // chưa có VBO (chưa có surface size)
            ...
            int vertexCount = RECT_COUNT * 6; // mỗi rect 6 đỉnh
            ...
            mainHandler.post(() -> {
                context.getSharedPreferences("results", Context.MODE_PRIVATE)
                        .edit()
                        .putFloat("opengl_render_time", (float) renderTimeMs)
                        .apply();
                Log.d("OpenGLTest", "OpenGL ES first-frame render: " + renderTimeMs + " ms");
                Toast.makeText(
                        context,
                        "OpenGL ES first frame: " + String.format("%.2f ms", renderTimeMs),
                        Toast.LENGTH_LONG
                ).show();
            });
